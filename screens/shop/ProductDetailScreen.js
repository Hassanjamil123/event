...
const ProductDetailScreen = (props) => {
  ...
  const fetchInterestedStateAndCount = async () => {
    try {
      ...
      const response = await axios.get(
        `https://event-950ef-default-rtdb.firebaseio.com/interestedEvents/${productId}.json?auth=${userToken}`
  ...
  const saveEventToDatabase = async (userId, eventId) => {
    try {
      ...
      const response = await fetch(
        `https://event-950ef-default-rtdb.firebaseio.com/savedEvents/${userId}/savedEvents.json?auth=${userToken}`,
        {
          method: 'POST',
          headers: {
  ...
  const removeEventFromDatabase = async () => {
    try {
      ...
      const response = await fetch(
        `https://event-950ef-default-rtdb.firebaseio.com/savedEvents/${userId}/savedEvents/${productId}.json?auth=${userToken}`,
        {
          method: 'DELETE',
        }
      );
