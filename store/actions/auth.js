  ...
  return async (dispatch) => {
    try {
      ...
      const response = await fetch(
        `https://event-950ef-default-rtdb.firebaseio.com/savedEvents/${userId}`,
        {
          headers: {
...
const storeUserData = async (uid, userData) => {
  try {
    const response = await fetch(
      `https://event-950ef-default-rtdb.firebaseio.com/users/${uid}.json`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await axios.post(
        'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyB_-YjoRXMRJG4isEw_jtkLnZd0mlCgQC0',
      ...
      await axios.put(
        `https://event-950ef-default-rtdb.firebaseio.com/users/${localId}.json`,
        userData,
        {
          params: { auth: idToken },
        }
...
export const orgSignup = (name, phone, email, password) => {
  return async (dispatch) => {
    try {
      ...
      await axios.put(
        `https://event-950ef-default-rtdb.firebaseio.com/eventOrganizers/${localId}.json`,
        userData,
        {
          params: { auth: idToken },
        }
...
export const login = (email, password) => {
  return async (dispatch) => {
    try {
      ...
      const response = await axios.post(
        'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyB_-YjoRXMRJG4isEw_jtkLnZd0mlCgQC0',
      ...
      navigation.navigate('PrductsOverviewo');
