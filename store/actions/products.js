  ...
  return async (dispatch) => {
    try {
      ...
      const response = await fetch(
        'https://event-950ef-default-rtdb.firebaseio.com/events.json'
...
export const deleteProduct = productId => {
  return async (dispatch, getState) => {
    ...
    const response = await fetch(
      `https://event-950ef-default-rtdb.firebaseio.com/events/${productId}.json?auth=${token}`,
      {
        method: 'DELETE'
      }
    );
...
export const createProduct = (title, description, imageUrl, price, location, date, category, viewCount) => {
  return async (dispatch, getState) => {
    ...
    const response = await fetch(
      `https://event-950ef-default-rtdb.firebaseio.com/events.json?auth=${token}`,
      {
        method: 'POST',
        headers: {
...
export const updateProduct = (id, title, description, imageUrl, location, date) => {
  return async (dispatch, getState) => {
    const token = getState().auth.token;
    const response = await fetch(
      `https://event-950ef-default-rtdb.firebaseio.com/events/${id}.json?auth=${token}`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
...
export const fetchOrganizerProducts = () => {
  return async (dispatch, getState) => {
    ...
    try {
      const response = await fetch(
        `https://event-950ef-default-rtdb.firebaseio.com/events/${userId}/events.json?auth=${token}`
      );
